...
class EntityCommonSerializer(serializers.ModelSerializer):
    ...
    def render_html(self, entity, postfix):
        ...
        cache_key = 'entity:{0}|{1}-{2}-{3}-{4}-{5}'.format(entity.id, app_label, self.label, entity_type, postfix, get_language_from_request(request))
        content = cache.get(cache_key)
        if content:
            return mark_safe(content)
        params = [
            (app_label, self.label, entity_type, postfix),
...
class SerializerRegistryMetaclass(serializers.SerializerMetaclass):
    ...
    different polymorphic entity types in the Catalog view.
    """
...
class EntitySummarySerializerBase(with_metaclass(SerializerRegistryMetaclass, EntityCommonSerializer)):
    """
    Serialize a summary of the polymorphic Entity model, suitable for Catalog List Views etc.
    """
...
class EntitySelectSerializer(serializers.ModelSerializer):
    """
    A simple serializer to convert the entities name and code for rendering the select widget
    when looking up for a entity.
    """
...
class BaseItemSerializer(ItemModelSerializer):
    url = serializers.HyperlinkedIdentityField(lookup_field='pk', view_name='edw:cart-detail')
    ...
    def validate_product(self, product):
        if not product.active:
            msg = "Object `{}` is inactive, and can not be added to the cart."
