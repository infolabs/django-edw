...
def _get_terms_ids(entities_qs: 'BaseEntityQuerySet', tree: Any) -> List[int]:
    ...
    Функция выполняет:
    1. Проверяет, кэширован ли результат в наборе запросов
    2. Если не кэширован, вычисляет результат путем:
       - Получения связанных идентификаторов терминов из набора запросов
       - Обрезки дерева на основе этих идентификаторов
       - Преобразования ключей дерева в список
    3. Кэширует результат в наборе запросов для последующего использования
    
    Args:
        entities_qs: Набор запросов сущностей для получения терминов
        tree: Дерево терминов для обрезки и валидации
        
    Returns:
        List[int]: Список идентификаторов терминов, действительных для данного набора запросов сущностей
    """
