...
class EntityDetailSerializerBase(EntityDynamicMetaMixin,
    ...
    def _update_entity(self, instance, is_created, validated_data):
        """
        Обновляет связанное состояние сущности на основе входных данных сериализатора.

        Последовательно обрабатывает данные из `validated_data` и синхронизирует
        связанные сущности и связи для `instance`.

        Логика работы с `characteristics` и `marks`:
        - для каждого блока атрибутов извлекает список через `pop`, поэтому поля
          считаются потребленными и не передаются дальше в стандартный `update`;
        - ищет родительский терм атрибута по `slug` (если в `path` нет `/`) либо
          по полному `path` (если `/` есть);
        - при ошибке поиска (нет терма или найдено несколько) выбрасывает
          `ValidationError`;
        - для каждого значения в `values` пытается сопоставить его с дочерним термом
          найденного атрибута:
          - если совпадение найдено, id дочернего терма добавляется в `attr_terms_ids`,
            а одноименная запись в `AdditionalEntityCharacteristicOrMarkModel`
            удаляется (т.к. значение представимо через термин);
          - если совпадения нет, значение хранится как "дополнительное" строковое
            значение через `update_or_create` в
            `AdditionalEntityCharacteristicOrMarkModel`;
        - если `values` пуст, удаляются все дополнительные значения для данного
          атрибута и сущности.

        После обработки характеристик/меток:
        - добавляет термы из `terms_paths` и/или `active_terms_ids`;
        - при необходимости учитывает ограничения и обязательные термы витрины данных;
        - назначает итоговый набор терминов в `instance.terms.set(...)`;
        - пересобирает отношения (`relations`), включая данные по субъектам и
          ограничения витрины данных для прямых/обратных связей.

        Аргументы:
            instance: экземпляр сущности, который нужно обновить.
            is_created: признак того, что сущность только что создана
                (в этом случае выполняется инициализация терминов/связей по умолчанию).
            validated_data: валидированные входные данные, из которых функция извлекает
                и удаляет (`pop`) обслуживаемые поля.
        """
        attr_terms_ids = []
        # characteristics, marks
        for (attr_name, attribute_mode) in [
