...
def on_pre_delete_postzone(sender, instance, **kwargs):
    ...
    entities_ids = EntityModel.objects.instance_of(*_model_with_place_minix).filter(terms__id=zone_term_id).values_list(
        'id', flat=True)
    ...
    if instance.pk is not None:
        try:
            ...
            if origin.term != instance.term:
                ...
                zone_term_id= instance.term.id
                entities_ids = EntityModel.objects.instance_of(*_model_with_place_minix).filter(
